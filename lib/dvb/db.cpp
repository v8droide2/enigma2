	...
	switch(line[0])
	{
		case 's':
		{
			...
			sscanf(line+2, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d",
				...
				&is_id, &pls_code, &pls_mode, &t2mi_pid, &plp_code);
			...
			while (options) {
				...
				if (strncmp(options, "T2MI/PLP:", 5) == 0)
					sscanf(options+5, "%d:%d", &t2mi_pid, &plp_code);
				options = next;
			}
			sat.is_id = is_id;
...
void eDVBDB::saveServicelist(const char *file)
{
	...
	if (g) {
		...
		fprintf(g, "#     DVBS2 FEPARMS:   s:frequency:symbol_rate:polarisation:fec:orbital_position:inversion:flags:system:modulation:rolloff:pilot[,MIS/PLS:is_id:pls_code:pls_mode][,T2MI/PLP:t2mi_pid:plp_code]\n");
			...
			i != m_channels.end(); ++i)
	{
		...
		if (!ch.m_frontendParameters->getDVBS(sat))
		{
			...
			if (sat.system == eDVBFrontendParametersSatellite::System_DVB_S2)
			{
				...
				if (static_cast<unsigned int>(sat.t2mi_pid) != 0 || (sat.plp_code) != eDVBFrontendParametersSatellite::PLP_Code)
				{
					fprintf(f, ":%d:%d", sat.t2mi_pid, sat.plp_code);
					if (g)
						fprintf(g, ",T2MI/PLP:%d:%d", sat.t2mi_pid, sat.plp_code);
				}
...
PyObject *eDVBDB::readSatellites(ePyObject sat_list, ePyObject sat_dict, ePyObject tp_dict)
{
	...
	int tmp, *dest = NULL,
		modulation, system, freq, sr, pol, fec, inv, pilot, rolloff, is_id, pls_code, pls_mode, t2mi_pid, plp_code, tsid, onid;
	char *end_ptr;
		...
		if (sat_pos && sat_name)
		{
			...
			while(transponder)
			{
				...
				for(xmlAttrPtr attr = transponder->properties; attr; attr = attr->next)
				{
					...
					else if (name == "plp_code") dest = &plp_code;
